# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 18:03:24 2020
...
def infeccion():
    ...
    for inf in contagiadosvisibles+contagiadosnovisibles:
        for san in reversed(sanos):
           if ((inf.x-san.x)**2+(inf.y-san.y)**2)**0.5<radiocontagio.val :
               if random.randint(1,100)<=100*ProbabilidadDeContagio.val:
                  san.TiempoDeContagio=pygame.time.get_ticks()
                  if random.randint(1,100)<=100*porcentajeasimptomatico.val: 
...
def dibujar():  # mostrar círculos
    ...
    for i in contagiadosvisibles+contagiadosnovisibles:
        pygame.draw.circle(pantalla,(5, 153, 29),(int(i.x),int(i.y)),radiocontagio.val)
    for c in sanos+contagiadosvisibles+contagiadosnovisibles+retirados:
...
radiocontagio = Slider("radio infeccion", 6, 10, 5, 100)
ProbabilidadDeContagio = Slider("Contagio", 0.5, 1, 0, 200)
TamañoDeLaPoblacion = Slider("Poblacion", 250, 500, 250, 300)
porcentajeasimptomatico = Slider("Asintomatismo", 0, 1, 0, 400)
slides = [velocidad, radiocontagio, ProbabilidadDeContagio, TamañoDeLaPoblacion, porcentajeasimptomatico]
